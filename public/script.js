...
function checkVideoTime(video) {
    ...
    if (remainingTime <= 16) { // 비디오 종료 5.64초 전
        enterButton.classList.remove('hidden');
        enterButton.style.display = 'block';
        enterButton.style.opacity = '1';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isMobile) {
        ...
        introVideoMobile.addEventListener('play', showSkipButton);
    } else {
        introVideo.addEventListener('ended', handleVideoEnd);
        introVideo.addEventListener('timeupdate', () => {
            checkVideoTime(introVideo);
        });
        introVideo.addEventListener('play', showSkipButton);
    }
});
...
enterButton.addEventListener('click', () => {
    ...
    window.location.href = './cocooon-gallery/dist/index.html';
});
...
testButton.addEventListener('click', () => {
    ...
    if (video.duration) {
        video.currentTime = video.duration - 18; // 영상 끝나기 10초 전으로 이동
    }
});

// 비디오 종료 시 스킵 버튼 타이머 정리
if (isMobile) {
    introVideoMobile.addEventListener('ended', () => {
